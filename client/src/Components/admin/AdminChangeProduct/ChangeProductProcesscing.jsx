...
const ChangeProductProcesscing = () => {
    ...
    const handleSearch = (query, page = 1) => {
        setLoading(true);
        axios.get(`${process.env.REACT_APP_API_URL}/order/searchOrdersWithChangeStatus?q=${query}&page=${page}`, { headers })
            .then((response) => {
    ...
    const fetchOrders = (page = 1) => {
        axios
            .get(`${process.env.REACT_APP_API_URL}/order/orders-with-status?page=${page}`, { headers })
            .then((response) => {
                setSearchResults(null);
...
export default ChangeProductProcesscing;
