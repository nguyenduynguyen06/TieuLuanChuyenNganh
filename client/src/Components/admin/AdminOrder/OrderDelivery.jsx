...
const OrderDelivery = () => {
    ...
    useEffect(() => {
        if (searchQuery.trim() !== '') {
            axios.get(`${process.env.REACT_APP_API_URL}/order/searchOrderDelivery?orderCode=${searchQuery}`, { headers })
                .then((response) => {
    ...
    const fetchOrders = (page) => {
        axios
            .get(`${process.env.REACT_APP_API_URL}/order/getAllOrdersDelivery?page=${page}`, { headers })
            .then((response) => {
    ...
    const fetchOrdersWithShippingMethod = (shippingMethod, page) => {
        axios
            .get(`${process.env.REACT_APP_API_URL}/order/getAllOrdersDelivery?page=${page}&shippingMethod=${shippingMethod}`, { headers })
            .then((response) => {
                setOrderData(response.data.data);
                setTotalPages(response.data.pageInfo.totalPages);
                setSearchResults(null);
    ...
    const onConfirm2 = () => {
        updateOrderDelivery(selectedOrder._id);
        setConfirmModalVisible2(false);
        setSelectedOrder(null);
    };
    ...
    const updateOrderDelivery = (orderId) => {
        axios
        ...
        if (order.shippingMethod === 'Nhận tại cửa hàng') {
            return (
                <Menu onClick={(e) => handleMenuClick(e, order)}>
                    <Menu.Item key="confirm">Hoàn thành đơn hàng</Menu.Item>
    ...
    return (
        <div>
            ...
            {selectedOrder && (
                ...
                >
                    Bạn có muốn hoàn thành đơn hàng {selectedOrder.orderCode} này không?
                </Modal>
            )}
            {selectedOrder && (
                <Modal
            ...
            {selectedOrder && (
                <Modal
                    ...
                    footer={[
                        ...
                        <Button key="submit" type="primary" onClick={onConfirm2}>
                            Xác nhận
                        </Button>,
                    ]}
                >
                    Bạn có muốn xác nhận đã giao đơn hàng {selectedOrder.orderCode} này không?
                </Modal>
            )}
        </div>
    );
