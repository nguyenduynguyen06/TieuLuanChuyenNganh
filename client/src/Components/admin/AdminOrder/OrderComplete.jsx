...
import { AppstoreFilled, PrinterFilled } from '@ant-design/icons';
...
const OrderComplete = () => {
    ...
    useEffect(() => {
        if (searchQuery.trim() !== '') {
            axios.get(`${process.env.REACT_APP_API_URL}/order/searchOrderComplete?orderCode=${searchQuery}`, { headers })
                .then((response) => {
    ...
    const fetchOrders = (page) => {
        axios
            .get(`${process.env.REACT_APP_API_URL}/order/getAllOrdersComplete?page=${page}`, { headers })
            .then((response) => {
    ...
    const fetchOrdersWithShippingMethod = (shippingMethod, page) => {
        axios
            .get(`${process.env.REACT_APP_API_URL}/order/getAllOrdersComplete?page=${page}&shippingMethod=${shippingMethod}`, { headers })
            .then((response) => {
                setOrderData(response.data.data);
                setTotalPages(response.data.pageInfo.totalPages);
                setSearchResults(null);
        ...
        <div>
                    ...
                    <MDBTable bordered align='middle' className='floating-table'>
                        <MDBTableHead>
                            ...
                            <th scope='col' style={{ verticalAlign: 'middle', minWidth: '150px' }}>Mã đơn hàng</th>
                                ...
                                <th scope='col' style={{ verticalAlign: 'middle', minWidth: '300px' }}>Thời gian hoàn thành đơn hàng</th>
                                <th scope='col' style={{ verticalAlign: 'middle', minWidth: '150px' }}>Thao tác</th>                            </tr>
                        </MDBTableHead>
                        <MDBTableBody>
                            ...
                            {(!searchResults && orderData.length === 0) ? (
                                ...
                                <tr style={{ textAlign: 'center' }}>
                                    ...
                                    <td>{searchResults.completeDate}</td>
                                    <td>
            ...
            <>
                <Modal
                    ...
                    maskClosable={false}
                >
                    <ProductOrder closeModal={closeModal2} orderId = {orderId} items={selectedOrderItems} orderCode={orderCode} orderVoucher={orderVoucher} orderSubtotal={orderSubtotal} ordertotalPay={ordertotalPay} orderStatus ={orderStatus}/>
                </Modal>
            </>
