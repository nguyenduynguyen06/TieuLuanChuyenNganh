...
const OrderComplete = () => {
    ...
    const handleSearch = (query, shippingMethod, page = 1) => {
        setLoading(true);
        axios.get(`${process.env.REACT_APP_API_URL}/order/searchOrderComplete?q=${query}&page=${page}&shippingMethod=${shippingMethod}`, { headers })
            .then((response) => {
        ...
        <div>
            ...
            <>
                <Modal
                    ...
                    maskClosable={false}
                >
                    <ProductOrder closeModal={closeModal2} orderId={orderId} items={selectedOrderItems} orderCode={orderCode} orderVoucher={orderVoucher} orderSubtotal={orderSubtotal} ordertotalPay={ordertotalPay} orderStatus={orderStatus} orderShippingFee={orderShippingFee}/>
                </Modal>
            </>
