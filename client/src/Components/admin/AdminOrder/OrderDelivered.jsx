...
const OrderDelivered = () => {
    ...
    useEffect(() => {
        if (searchQuery.trim() !== '') {
            axios.get(`${process.env.REACT_APP_API_URL}/order/searchOrderDelivered?orderCode=${searchQuery}`, { headers })
                .then((response) => {
    ...
    const fetchOrdersWithShippingMethod = (shippingMethod, page) => {
        axios
            .get(`${process.env.REACT_APP_API_URL}/order/getAllOrdersDelivered?page=${page}&shippingMethod=${shippingMethod}`, { headers })
            .then((response) => {
                setOrderData(response.data.data);
                setTotalPages(response.data.pageInfo.totalPages);
                setSearchResults(null);
        ...
        <div>
            ...
            <div style={{ marginTop: '10px', display: 'flex', alignItems: 'center' }}>
                <div style={{ marginRight: '10px' }}>
                    ...
                    >
                        <option value="">Tất cả đơn hàng đã giao</option>
                    </select>
                </div>
                <div>
