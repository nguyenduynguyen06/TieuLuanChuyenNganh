...
import { Button, Popover, Rate, Skeleton, Tooltip, message } from 'antd';
...
function ProductHP() {
    ...
    const handleAddToCart = async (productName, memory) => {
        // try {
        //     const selectedVariant = productDetails.variant.find((variant) => variant.memory === memory);
        //     if (selectedVariant) {
        //         const selectedSKUName = selectedVariant._id; 
        //         await axios.post(`${process.env.REACT_APP_API_URL}/cart/addCart?userId=${user._id}&productName=${productName}&SKU=${selectedSKUName}&quantity=1`);
    ...
    const handleBuyNow = async (productName, memory) => {
        // try {
        //     const selectedVariant = productDetails.variant.find((variant) => variant.memory === memory);
        //     if (selectedVariant) {
        //         const selectedSKUName = selectedVariant._id; 
        ...
        // } catch (error) {
        //     console.error('Lỗi:', error);
        //     message.error('Đã xảy ra lỗi khi thực hiện thanh toán');
        // }
    };
    ...
    return (
        <ProductHPStyle>
            {loading ? (
                <Skeleton style={{ padding: 10, minHeight: '340px' }} active={true} />
            ) : (
